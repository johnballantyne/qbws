 ...
 *          or the original input parameter if it does not match the regular
 *          expression
 */
...
/**
 * Checks that the type of a variable is a 'object' and that it is not empty
 ...
 *              the current web connector version and react to it. Not
 *              currently required to support backward compatibility but
 *              strongly recommended.
 *
 *              Supply one of the following return strings:
 *              - "NULL" or "" (empty string) if you want the Web Connector to
 *                proceed with the update
 *              - "W:<any text>" if you want the web Connector to display a
 *                WARNING dialog prompting the user to continue with the update
 *                or cancel it. The text string after the "W:" will be
 *                displayed in the warning dialog.
 *              - "E:<any text>" if you want to cancel the update and display
 *                an ERROR dialog. The text string after "E:" will be displayed
 *                in the error dialog. The user will have to download a new
 *                version of the Web Connector to continue with the update.
 *              - "O:<version number>" to tell the user that the server expects
 *                a newer version of QBWC than the user currently has but also
 *                tells the user which version is needed
 * @param strVersion {String} The version of the QB web connector supplied in
 *        the web connector's call to clientVersion
...
/**
 * @summary Verifies the username and password for the Web Connector that is
 *          trying to connect
...
/**
 * @summary This web method facilitates qbws to receive the response XML from
 *          QuickBooks via the Web Connector
 * @param ticket {String}
 * @param response {String}
 * @param hresult {String}
 * @param message {String}
 * @returns {Number} The percentage of work done. Must be an integer.
 *          - > 0 - There are more requests to sends
